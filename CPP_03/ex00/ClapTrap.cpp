...
void    ClapTrap::takeDamage(unsigned int amount)
{
    ...
    else
    {
        if (Hit_points - amount >= 0)
            ...
            Hit_points = 0;
        std::cout << "ClapTrap " << name << " took " << amount << " hit points, remaining points " << Hit_points << std::endl;
    }
}
...
void    ClapTrap::beRepaired(unsigned int amount)
{
    if (Energy_points > 0 && Hit_points > 0)
    {
        Hit_points += amount;
        Energy_points--;
        std::cout << "ClapTrap " << name << " Repaired " << amount << " hit points, remaining points " << Hit_points << std::endl;
    }
    else
